...
def main(file_path, window_size, interval_timestamps, timestamps, k, metric):
    ...
    train_k_similar_data_for_all_query = get_k_similar_data_for_all_query(train_reference_features, train_reference_targets, train_query_features, train_query_targets, k, metric)
    print("Calculation Ended")
    print(datetime.datetime.now())
    file_name = f"prepared_data_similar_trajectories/train_{k}_similar_data_for_all_query_with_window_size_{window_size}_and_metric_{str_metric}.npy"
